...
Tx = generate_Tx(T_PRI, Fmin, Tchirp, Nc, s);                                           % Generating the Transmitted Signal of the radar
Rx = simulate_Rx(T_total, T_chirp, Fmin, s, PRI, Nc, Range, c, Fdoppler, L, N_time);    % Simulating the Received Signal from objects
...
Zt = Beat_mixer(Tx, Rx, L);                                % Mixer to Get Beat Frequency                     
Beat_signal = Beat_lpf(Zt, Fs, T_plot, N_plot, Colors, L); % Low Pass Filter for Better Data
...
[FFT_beat, f_axis] = range_fft_one_chirp(Beat_signal, T_total, Tchirp, Fs, L);      % FFT for fast-axis
