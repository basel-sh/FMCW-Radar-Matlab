...
plot(velocity_axis, velocity_spectrum, 'k', ...
     'LineWidth', 1.6, ...
     'DisplayName', 'Velocity Spectrum');

minPeakProm = 6;
[pks, locs] = findpeaks( ...
    velocity_spectrum, ...
    'MinPeakProminence', minPeakProm, ...
    'MinPeakDistance', round(length(velocity_axis)/20));
...
for k = 1:length(V_est)
    ...
    xline(V_est(k), '--', ...
        'Color', colors(k,:), ...
        'LineWidth', 1.8, ...
        'DisplayName', sprintf('Peak %d: %.2f m/s', k, V_est(k)));

    plot(V_est(k), pks(k), 'o', ...
        'Color', colors(k,:), ...
        'MarkerFaceColor', colors(k,:), ...
        'MarkerSize', 7, ...
        'HandleVisibility','off');

    text(V_est(k), pks(k) + 2, ...
...
ylim([min(velocity_spectrum)-5, max(velocity_spectrum)+12]);
